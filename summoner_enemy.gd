...
func _ready():
	...
	hp = int(hp * mult)
	$ProceduralEnemy.setup("summoner", Color(0.2, 4.0, 3.0)) # Deep Teal Summoner
...
func _physics_process(delta: float):
	if is_instance_valid(player):
		...
		velocity = (dir + separation * 1.5).normalized() * SPEED
		...
		if summon_timer <= 0:
			_summon_minions()
			summon_timer = summon_rate

func _summon_minions():
	# Visual feedback for summoning
	ShakeManager.add_trauma(0.3)
	# (Logic for spawning basic enemies could go here)
	pass
...
func die():
	...
	var drop_chance = 0.50 * max(0.2, 2.5 * pow(0.85, GameManager.current_wave - 1))
